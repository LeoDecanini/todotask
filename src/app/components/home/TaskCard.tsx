...
const TaskCard = ({ task }: any) => {
  ...
  const { deleteTask, statusTask } = useVariables();
  ...
  return (
    <div className={`bg-white border-2 ${task.status === "Pendiente" ? "border-red-400" : "border-green-400"} group text-left hover:bg-slate-50 transition-all cursor-pointer flex flex-col justify-between text-gray-800 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.2)] max-h-[300px] rounded-xl p-5`}>
      <div>
          ...
          <div className="flex gap-4 text-3xl">
            <Dialog
              ...
              onOpenChange={(isOpen) => {
                setOpen2(isOpen);
                if (isOpen) {
                  route.push(`?editing=true&taskId=${task._id}`);
                } else {
                  route.push(`?editing=false&taskId=""`);
                }
              }}
            >
